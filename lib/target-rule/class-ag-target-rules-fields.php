<?php
/**
 * Ag_Target_Rules_Fields
 *
 * @package   Ag
 */

namespace Ag_Theme_Builder\Lib;

/**
 * Meta Boxes setup
 */
class Ag_Target_Rules_Fields {
	...
	public function __construct() {
		define( 'AG_TARGET_RULE_URI', plugins_url( '/', __FILE__ ) );
		define( 'AG_TARGET_RULE_VER', '1.0.0' );
		define( 'AG_TARGET_RULE_DIR', plugin_dir_path( __FILE__ ) );

		add_action( 'admin_action_edit', array( $this, 'initialize_options' ) );
		add_action( 'wp_ajax_ag_theme_builder_get_posts_by_query', array( $this, 'ag_theme_builder_get_posts_by_query' ) );
	}
	...
	public static function get_location_selections() {
		...
		$post_types = apply_filters( 'ag_location_rule_post_types', array_merge( $post_types, $custom_post_type ) );

		$special_pages = array(
			'special-404'    => __( '404 Page', 'ag-theme-builder' ),
			'special-search' => __( 'Search Page', 'ag-theme-builder' ),
			'special-blog'   => __( 'Blog / Posts Page', 'ag-theme-builder' ),
			'special-front'  => __( 'Front Page', 'ag-theme-builder' ),
			'special-date'   => __( 'Date Archive', 'ag-theme-builder' ),
			'special-author' => __( 'Author Archive', 'ag-theme-builder' ),
		);

		if ( class_exists( 'WooCommerce' ) ) {
			$special_pages['special-woo-shop'] = __( 'WooCommerce Shop Page', 'ag-theme-builder' );
		}

		$selection_options = array(
			'basic'         => array(
				'label' => __( 'Basic', 'ag-theme-builder' ),
				'value' => array(
					'basic-global'    => __( 'Entire Website', 'ag-theme-builder' ),
					'basic-singulars' => __( 'All Singulars', 'ag-theme-builder' ),
					'basic-archives'  => __( 'All Archives', 'ag-theme-builder' ),
				),
			),

			'special-pages' => array(
				'label' => __( 'Special Pages', 'ag-theme-builder' ),
				'value' => $special_pages,
			),
		);
		...
		$selection_options['specific-target'] = array(
			'label' => __( 'Specific Target', 'ag-theme-builder' ),
			'value' => array(
				'specifics' => __( 'Specific Pages / Posts / Taxonomies, etc.', 'ag-theme-builder' ),
			),
		);
		 ...
		 * @since 1.0.0
		 */
		return apply_filters( 'ag_display_on_list', $selection_options );
	}
	...
	public static function get_user_selections() {
		$selection_options = array(
			'basic'    => array(
				'label' => __( 'Basic', 'ag-theme-builder' ),
				'value' => array(
					'all'        => __( 'All', 'ag-theme-builder' ),
					'logged-in'  => __( 'Logged In', 'ag-theme-builder' ),
					'logged-out' => __( 'Logged Out', 'ag-theme-builder' ),
				),
			),

			'advanced' => array(
				'label' => __( 'Advanced', 'ag-theme-builder' ),
				'value' => array(),
			),
		);
		 ...
		 * @since 1.0.0
		 */
		return apply_filters( 'ag_user_roles_list', $selection_options );
	}
	 ...
	 * @since  1.0.0
	 */
	public function ag_theme_builder_get_posts_by_query() {

		check_ajax_referer( 'ag-theme-builder-get-posts-by-query', 'nonce' );
	 ...
	 * This is only run from ag_theme_builder_get_posts_by_query()
	 *
	...
	public function admin_styles() {
		wp_enqueue_script( 'ag-select2', AG_TARGET_RULE_URI . 'select2.js', array( 'jquery' ), AG_TARGET_RULE_VER, true );
		...
		if ( '' !== $wp_lang ) {
			...
			if ( isset( $select2_lang[ $wp_lang ] ) && file_exists( AG_TARGET_RULE_DIR . 'i18n/' . $select2_lang[ $wp_lang ] . '.js' ) ) {
				$ag_lang = $select2_lang[ $wp_lang ];
				wp_enqueue_script(
					'ag-select2-lang',
					AG_TARGET_RULE_URI . 'i18n/' . $select2_lang[ $wp_lang ] . '.js',
					array(
						'jquery',
						'ag-select2',
					),
		...
		wp_register_script(
			'ag-target-rule',
			AG_TARGET_RULE_URI . 'target-rule.js',
			array(
				'jquery',
				'wp-util',
				'ag-select2',
		...
		wp_enqueue_script( 'ag-target-rule' );

		wp_register_script(
			'ag-user-role',
			AG_TARGET_RULE_URI . 'user-role.js',
			array(
				'jquery',
			),
			AG_TARGET_RULE_VER,
		...
		wp_enqueue_script( 'ag-user-role' );

		wp_register_style( 'ag-select2', AG_TARGET_RULE_URI . 'select2.css', '', AG_TARGET_RULE_VER );
		wp_enqueue_style( 'ag-select2' );
		wp_register_style( 'ag-target-rule', AG_TARGET_RULE_URI . 'target-rule.css', '', AG_TARGET_RULE_VER );
		wp_enqueue_style( 'ag-target-rule' );
		...
		$localize_vars = array(
			'ag_lang'     => $ag_lang,
			'please_enter'  => __( 'Please enter', 'ag-theme-builder' ),
			'please_delete' => __( 'Please delete', 'ag-theme-builder' ),
			'more_char'     => __( 'or more characters', 'ag-theme-builder' ),
			'character'     => __( 'character', 'ag-theme-builder' ),
			'loading'       => __( 'Loading more results…', 'ag-theme-builder' ),
			'only_select'   => __( 'You can only select', 'ag-theme-builder' ),
			'item'          => __( 'item', 'ag-theme-builder' ),
			'char_s'        => __( 's', 'ag-theme-builder' ),
			'no_result'     => __( 'No results found', 'ag-theme-builder' ),
			'searching'     => __( 'Searching…', 'ag-theme-builder' ),
			'not_loader'    => __( 'The results could not be loaded.', 'ag-theme-builder' ),
			'search'        => __( 'Select Specific', 'ag-theme-builder' ),
			'ajax_nonce'    => wp_create_nonce( 'ag-theme-builder-get-posts-by-query' ),
		);
		wp_localize_script( 'ag-select2', 'AgRules', $localize_vars );
	}
	...
	public static function target_rule_settings_field( $name, $settings, $value ) {
		...
		$add_rule_label = isset( $settings['add_rule_label'] ) ? $settings['add_rule_label'] : __( 'Add Rule', 'ag-theme-builder' );
		$saved_values   = $value;
		$output         = '';
		...
		/* WP Template Format */
		$output .= '<script type="text/html" id="tmpl-ag-target-rule-' . $rule_type . '-condition">';
		$output .= '<div class="ag-target-rule-condition ag-target-rule-{{data.id}}" data-rule="{{data.id}}" >';
		...
		$output .= '<select name="' . esc_attr( $input_name ) . '[rule][{{data.id}}]" class="target_rule-condition form-control ag-input">';
		$output .= '<option value="">' . __( 'Select', 'ag-theme-builder' ) . '</option>';
		...
		$output .= '</div> <!-- ag-target-rule-condition -->';

		/* Specific page selection */
		$output .= '<div class="target_rule-specific-page-wrap d-none">';
		$output .= '<select name="' . esc_attr( $input_name ) . '[specific][]" class="target-rule-select2 target_rule-specific-page form-control ag-input " multiple="multiple">';
		$output .= '</select>';
		$output .= '</div>';
		...
		/* Wrapper Start */
		$output .= '<div class="ag-target-rule-wrapper ag-target-rule-' . $rule_type . '-on-wrap" data-type="' . $rule_type . '">';
		$output .= '<div class="ag-target-rule-selector-wrapper ag-target-rule-' . $rule_type . '-on">';
	...
	public static function get_post_target_rule_options( $post_type, $taxonomy ) {
		...
		$all_posts                          = sprintf( __( 'All %s', 'ag-theme-builder' ), $post_label );
		$post_option[ $post_name . '|all' ] = $all_posts;

		if ( 'pages' !== $post_key ) {
			/* translators: %s post label */
			$all_archive                                = sprintf( __( 'All %s Archive', 'ag-theme-builder' ), $post_label );
		...
		if ( in_array( $post_type->name, $taxonomy->object_type, true ) ) {
			...
			$tax_archive = sprintf( __( 'All %s Archive', 'ag-theme-builder' ), $tax_label );
	...
	public static function generate_target_rule_selector( $type, $selection_options, $input_name, $saved_values, $add_rule_label ) {
		...
		foreach ( $saved_values['rule'] as $index => $data ) {
			$output .= '<div class="ag-target-rule-condition ag-target-rule-' . $index . '" data-rule="' . $index . '" >';
			/* Condition Selection */
			$output .= '<span class="target_rule-condition-delete dashicons dashicons-dismiss"></span>';
			$output .= '<div class="target_rule-condition-wrap" >';
			$output .= '<select name="' . esc_attr( $input_name ) . '[rule][' . $index . ']" class="target_rule-condition form-control ag-input">';
			$output .= '<option value="">' . __( 'Select', 'ag-theme-builder' ) . '</option>';
		...
		if ( 'display' === $type ) {
			...
			$output .= '<a href="#" class="button">' . __( 'Add Exclusion Rule', 'ag-theme-builder' ) . '</a>';
			$output .= '</div>';
		}
	...
	public function get_current_layout( $layout_id, $option ) {
		...
		$current_layout = apply_filters( 'ag_target_page_settings', $current_layout, $layout_id );
	...
	public static function target_user_role_settings_field( $name, $settings, $value ) {
		...
		/* WP Template Format */
		$output         .= '<script type="text/html" id="tmpl-ag-user-role-condition">';
			$output     .= '<div class="ag-user-role-condition ag-user-role-{{data.id}}" data-rule="{{data.id}}" >';
				$output .= '<span class="user_role-condition-delete dashicons dashicons-dismiss"></span>';
				/* Condition Selection */
				$output     .= '<div class="user_role-condition-wrap" >';
					$output .= '<select name="' . esc_attr( $input_name ) . '[{{data.id}}]" class="user_role-condition form-control ag-input">';
					$output .= '<option value="">' . __( 'Select', 'ag-theme-builder' ) . '</option>';
		...
		foreach ( $selection_options as $group => $group_data ) {
			...
			$output .= '</optgroup>';
					...
					$output .= '</select>';
				$output     .= '</div>';
			$output         .= '</div> <!-- ag-user-role-condition -->';
		$output             .= '</script>';
		...
		$output         .= '<div class="ag-user-role-wrapper ag-user-role-display-on-wrap" data-type="display">';
			$output     .= '<div class="ag-user-role-selector-wrapper ag-user-role-display-on">';
				$output .= '<div class="user_role-builder-wrap">';
		foreach ( $saved_values as $index => $data ) {
			$output     .= '<div class="ag-user-role-condition ag-user-role-' . $index . '" data-rule="' . $index . '" >';
				$output .= '<span class="user_role-condition-delete dashicons dashicons-dismiss"></span>';
				/* Condition Selection */
				$output     .= '<div class="user_role-condition-wrap" >';
					$output .= '<select name="' . esc_attr( $input_name ) . '[' . $index . ']" class="user_role-condition form-control ag-input">';
					$output .= '<option value="">' . __( 'Select', 'ag-theme-builder' ) . '</option>';
	...
	public function get_posts_by_conditions( $post_type, $option ) {
		...
		if ( is_array( self::$current_page_data ) && isset( self::$current_page_data[ $post_type ] ) ) {
			return apply_filters( 'ag_get_display_posts_by_conditions', self::$current_page_data[ $post_type ], $post_type );
		}
 ...
 */
Ag_Target_Rules_Fields::get_instance();
